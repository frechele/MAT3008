...
#define CONV_FLOAT(i) ((float)(i))
#define CONV_DOUBLE(i) ((double)(i))
...
int main() {
    ...
    float f32_eps, f32_epsneg, f32_xmin, f32_xmax;
    double f64_eps, f64_epsneg, f64_xmin, f64_xmax;

    machar_float(&ibeta, &it, &irnd, &ngrd, &machep, &negep, &iexp, &minexp, &maxexp, &f32_eps, &f32_epsneg, &f32_xmin, &f32_xmax);
    machar_double(&ibeta, &it, &irnd, &ngrd, &machep, &negep, &iexp, &minexp, &maxexp, &f64_eps, &f64_epsneg, &f64_xmin, &f64_xmax);

    printf("[Machine Accuracy by using machar]\n");
